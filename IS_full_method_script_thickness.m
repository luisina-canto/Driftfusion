...
thick_arr = [1e-5,2e-5,3e-5,4e-5,5e-5,6e-5];
...
for i = 1:length(thick_arr)
    par_struct(i) = par;
    
    par_struct(i).d(3) = thick_arr(i);
...
for i = 1:length(thick_arr)
    ...
    IS_script_plot_impedance_2(sol_IS_OC(i),i,thick_arr(i),length(thick_arr))
    
    % IS_script_plot_nyquist(IS_results)
    IS_script_plot_nyquist_2(sol_IS_OC(i),i,thick_arr(i),length(thick_arr))

    % IS_script_plot_phase(IS_results)
    IS_script_plot_phase_2(sol_IS_OC(i),i,thick_arr(i),length(thick_arr))
end 
